...
write.csv(pcaData, "results/pca_coordinates.csv", row.names = FALSE)
...
colnames(summary_df) <- c("Category", "Number_of_Genes")

write.csv(summary_df, "results/volcano_summary.csv", row.names = FALSE)
...
heatmap_genes <- rownames(res)[topgenes]
heatmap_table <- data.frame(Gene = heatmap_genes)

write.csv(heatmap_table, "results/heatmap_top20_genes.csv", row.names = FALSE)
...
up_top <- head(up, 20)
write.csv(up_top, "results/upregulated_genes_top20.csv", row.names = FALSE)
...
write.csv(down_top, "results/downregulated_genes_top20.csv", row.names = FALSE)
...
write.csv(go_df, "results/go_enrichment.csv", row.names = FALSE)
...
write.csv(kegg_df, "results/kegg_enrichment_full.csv", row.names = FALSE)
