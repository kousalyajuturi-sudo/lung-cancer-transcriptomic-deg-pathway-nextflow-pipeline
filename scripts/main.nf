...
params.index = "/home/dell/RNA_project/reference/grch38_index"
params.gtf   = "/home/dell/RNA_project/reference/gencode.v45.annotation.gtf"
...
workflow {
    ...
    DESEQ2_OUT= DESEQ2(
        counts_ch,
        file(params.meta),
        file("scripts/deseq_RNA.R")
    )

   // Add KEGG links process
   // KEGG_OUT = KEGG_LINKS(
   // DESEQ2_OUT.filter{ it.getName() == "deseq_results.csv" },
   // file("scripts/generate_kegg_links.R")
//)

   FINAL_REPORT(
    file("scripts/final_report_RNA.py"),
    DESEQ2_OUT
)
}
...
process TRIM {
    ...
    script:
    """
    fastp -i $reads -o ${reads.simpleName}_trimmed.fastq.gz --thread 1
    """
}
...
process ALIGN {
    ...
    script:
    """
    hisat2 -x ${params.index} -U $reads -S ${reads.simpleName}.sam -p 1
    """
}
